    ...
    for epoch in range(start_epoch,epoch_num-1):
        ...
        for batch_cnt, data in tqdm(enumerate(data_loader['train']), desc='Epoch {}'.format(epoch+1), 
            ...
            if step % checkpoint == 0:
                ...
                if Config.use_dcl:
                    print('loss=ce_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} + {:6.4f} + {:6.4f} '.format(loss.detach().item(), ce_loss.detach().item(), swap_loss.detach().item(), law_loss.detach().item()), flush=True)
                if Config.use_backbone:
                    print('loss=ce_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} '.format(loss.detach().item(), ce_loss.detach().item()), flush=True)
