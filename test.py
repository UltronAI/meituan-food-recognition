...
from utils.dataset_DCL import collate_fn4train, collate_fn4test, collate_fn4val, collate_fn4backbone, dataset
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if os.path.exists('test_prediction.txt'):
            os.remove('test_prediction.txt')
        ...
        for batch_cnt_val, data_val in enumerate(dataloader):
            ...
            for i in range(dataloader.batch_size):
                str1= str(img_name[i])+' '+str(top3[i])
                f.write(str1)
                f.write('\r\n')
        f.close()
